...
data = [256*np.random.rand(epoch_size, 64, 64, num_channels) for i in range(2)]
labels = np.zeros((epoch_size,num_classes))
recog_model.fit(data, labels,batch_size=batch_size)

data = [256*np.random.rand(epoch_size, 32, 32, num_channels) for i in range(2)]
labels = np.zeros((epoch_size,num_classes))
recog_model.fit(data, labels,batch_size=batch_size)
